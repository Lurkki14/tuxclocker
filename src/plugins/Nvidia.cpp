...
template <typename T> struct NvidiaGPU {
	std::string uuid;
	// std::string name;
	T identifier;
};
...
template <typename T> struct UnspecializedReadable {
		...
		for (const auto &rawNode : rawNodes) {
			...
			if_let(pattern(as<ReadableValue>(_)) = rawNode.func(data)) = [=, &retval] {
				auto readable = DynamicReadable(
				    [=] { return rawNode.func(data); }, rawNode.unit);
	...
	static std::vector<DeviceNode> staticToDeviceNodes(
		...
		for (const auto &rawNode : rawNodes) {
			if_let(pattern(as<ReadableValue>(arg)) =
				   rawNode.func(data)) = [=, &retval](ReadableValue value) {
				auto staticReadable = StaticReadable(value, rawNode.unit);
				auto devNode = DeviceNode{.name = rawNode.nodeName,
...
template <typename T> struct UnspecializedAssignable {
	...
	static std::vector<DeviceNode> toDeviceNodes(
	    std::vector<UnspecializedAssignable<T>> rawNodes, T devData, std::string uuid) {
		std::vector<DeviceNode> retval;
		for (auto &rawNode : rawNodes) {
			if_let(
			    pattern(some(arg)) = rawNode.assignableInfo(devData)) = [&](auto info) {
				// Create an individual Assignable from a generalized Assignable
				auto assignable = Assignable(
				    [=](auto arg) { return rawNode.func(devData, info, arg); },
				    info, [=]() { return rawNode.currentValueFunc(devData); },
				    rawNode.unit);
				auto node = DeviceNode{.name = rawNode.nodeName,
...
private:
	...
	std::optional<AssignmentError> nvctlWrite(
	    uint index, int mask, int target, int attribute, int value);
...
std::optional<AssignmentError> NvidiaPlugin::nvctlWrite(
    uint index, int mask, int target, int attribute, int value) {
	if (XNVCTRLSetTargetAttributeAndGetStatus(m_dpy, target, index, mask, attribute, value)) {
		// Success
		return std::nullopt;
	}
...
int NvidiaPlugin::nvctrlFanCount(uint index) {
	...
	if (!XNVCTRLQueryTargetBinaryData(m_dpy, NV_CTRL_TARGET_TYPE_GPU, index, 0,
		NV_CTRL_BINARY_DATA_COOLERS_USED_BY_GPU, &data, &data_len)) {
		return 0;
	}
...
NvidiaPlugin::NvidiaPlugin() : m_dpy() {
	...
	// Make sure that features accessed through the different libraries are assigned to the
	// correct GPUs
	...
	std::vector<UnspecializedReadable<uint>> rawNVCTRLNodes = {
	    {[&](uint index) {
		     return nvctlRead<double>(
			 index,
			 [&](uint index, int *value) {
				 return XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
				     index, 0, NV_CTRL_GPU_CURRENT_CORE_VOLTAGE, value);
			 },
			 [](int value) { return static_cast<double>(value) / 1000; });
	     },
		"mV", "Core Voltage",
		[](std::string uuid, uint) { return md5(uuid + "Core Voltage"); }}};

	std::vector<UnspecializedReadable<nvmlDevice_t>> rawNVMLClockNodes = {
	    {.func =
		    [](nvmlDevice_t dev) {
			    return nvmlRead<uint, uint>(
				dev,
				[](nvmlDevice_t dev, uint *value) {
					return nvmlDeviceGetClockInfo(
					    dev, NVML_CLOCK_GRAPHICS, value);
				},
				std::nullopt);
		    },
		.unit = "MHz",
		.nodeName = "Core Clock",
		.hash = [](std::string uuid, nvmlDevice_t) { return md5(uuid + "Core Clock"); }},
	    {.func =
		    [](nvmlDevice_t dev) {
			    return nvmlRead<uint, uint>(
				dev,
		...
		.hash = [](std::string uuid, nvmlDevice_t) { return md5(uuid + "Memory Clock"); }}

	};

	std::vector<UnspecializedReadable<nvmlDevice_t>> rawNVMLNodes = {
	    {[](nvmlDevice_t dev) {
		     return nvmlRead<uint, double>(
			 dev,
			 [](nvmlDevice_t dev, uint *value) {
				 return nvmlDeviceGetPowerUsage(dev, value);
			 },
			 [](uint val) { return static_cast<double>(val) / 1000; });
	     },
		"W", "Power Usage",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Power Usage"); }}};
	...
	std::vector<UnspecializedReadable<nvmlDevice_t>> rawNVMLUtilNodes = {
		...
		"%", "Core Utilization",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Core Utilization"); }},
	    {[](nvmlDevice_t dev) -> ReadResult {
		     nvmlUtilization_t value;
		     nvmlReturn_t err;
		     if ((err = nvmlDeviceGetUtilizationRates(dev, &value)) != NVML_SUCCESS)
		...
		"%", "Memory Utilization",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Memory Utilization"); }}};
	...
	std::vector<UnspecializedAssignable<uint>> rawNVCTRLVoltageAssignables = {
	    {.assignableInfo = [&](uint index) -> std::optional<AssignableInfo> {
		     ...
		     auto valid =
			 XNVCTRLQueryValidTargetAttributeValues(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			     index, 0, NV_CTRL_GPU_OVER_VOLTAGE_OFFSET, &values);
		     if (valid)
			     // uV -> mV
			     return Range<int>{
				 values.u.range.min / 1000, values.u.range.max / 1000};
		     return std::nullopt;
	     },
		.func =
		...
		.currentValueFunc = [&](uint index) -> std::optional<AssignmentArgument> {
			int value;
			auto valid = XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			    index, 0, NV_CTRL_GPU_OVER_VOLTAGE_OFFSET, &value);
			if (valid)
				// uV -> mV
				return value / 1000;
			return std::nullopt;
		...
		.hash = [](std::string uuid, uint) { return md5(uuid + "Core Voltage Offset"); }}};
	...
	std::vector<UnspecializedReadable<uint>> rawNVCTRLUtilNodes = {
	    {[&](uint index) -> ReadResult {
		     ...
		     if (!XNVCTRLQueryTargetStringAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU, index,
			     0, NV_CTRL_STRING_GPU_UTILIZATION, &val)) {
			     return ReadError::UnknownError;
		     }
		...
		[](std::string uuid, uint) { return md5(uuid + "PCIe Bandwidth Utilization"); }}};
	...
	std::vector<UnspecializedAssignable<nvmlDevice_t>> rawNVMLAssignables = {
		...
		[](nvmlDevice_t dev, AssignableInfo info,
		    AssignmentArgument a_arg) -> std::optional<AssignmentError> {
			std::optional<AssignmentError> retval = AssignmentError::InvalidType;
			if_let(pattern(as<double>(arg)) = a_arg) = [=, &retval](auto val) {
				/* TODO: this is an unsafe conversion, either add a template
				 * parameter for range, check the type here or handle the possible
				 * exception. */
				...
				case NVML_ERROR_NO_PERMISSION:
					retval = AssignmentError::NoPermission;
					break;
				default:
					retval = AssignmentError::UnknownError;
					break;
				}
			};
			return retval;
		...
		"W", "Power Limit",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Power Limit"); }}};
	...
	std::vector<UnspecializedAssignable<NVClockInfo>> rawNVCTRLClockAssignables = {
	    {[=](NVClockInfo info) -> std::optional<AssignableInfo> {
		     ...
		     if (XNVCTRLQueryValidTargetAttributeValues(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			     info.index, info.maxState, NV_CTRL_GPU_MEM_TRANSFER_RATE_OFFSET,
			     &values)) {
		...
		[=](NVClockInfo c_info) -> std::optional<AssignmentArgument> {
			int value;
			auto valid = XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			    c_info.index, c_info.maxState, NV_CTRL_GPU_MEM_TRANSFER_RATE_OFFSET,
			    &value);
			if (valid)
				return value / 2;
			return std::nullopt;
		},
		"MHz", "Memory Clock Offset",
		[](std::string uuid, NVClockInfo) { return md5(uuid + "Memory Clock Offset"); }},
			...
			try {
				...
				if (arg < range.min || arg > range.max)
					...
					return nvctlWrite(c_info.index, c_info.maxState,
					    NV_CTRL_TARGET_TYPE_GPU, NV_CTRL_GPU_NVCLOCK_OFFSET,
					    arg);
			} catch (std::bad_variant_access &) {
			}
			return retval;
		...
		[=](NVClockInfo c_info) -> std::optional<AssignmentArgument> {
			int value;
			auto valid = XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			    c_info.index, c_info.maxState, NV_CTRL_GPU_NVCLOCK_OFFSET, &value);
			if (valid)
				return value;
			return std::nullopt;
		},
		"MHz", "Core Clock Offset",
		[](std::string uuid, NVClockInfo) { return md5(uuid + "Core Clock Offset"); }}
	...
	std::vector<UnspecializedAssignable<uint>> rawNVCTRLFanModeAssignables = {
	    {.assignableInfo = [=](uint index) -> std::optional<AssignableInfo> {
		     NVCTRLAttributeValidValuesRec values;
		     if (XNVCTRLQueryValidTargetAttributeValues(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			     index, 0, NV_CTRL_GPU_COOLER_MANUAL_CONTROL, &values) &&
		    ...
		    [=](uint index, AssignableInfo, AssignmentArgument a_arg) {
			    ...
			    if_let(pattern(as<uint>(arg)) = a_arg) = [=, &ret](auto u) {
				    ...
				    case 1:
					    ...
					    break;
				    default:
					    ret = AssignmentError::OutOfRange;
				    }
			    };
			    return ret;
		    },
		.currentValueFunc = [=](uint index) -> std::optional<AssignmentArgument> {
			int value;
			auto valid = XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			    index, 0, NV_CTRL_GPU_COOLER_MANUAL_CONTROL, &value);
			if (valid)
				return (value == NV_CTRL_GPU_COOLER_MANUAL_CONTROL_FALSE) ? 0u : 1u;
			return std::nullopt;
		},
		.unit = std::nullopt,
		.nodeName = "Fan Mode",
		.hash = [](std::string uuid, uint) { return md5(uuid + "Fan Mode"); }}};
	...
	std::vector<UnspecializedReadable<NVMLFanInfo>> rawNVMLFanReadables = {
	    {[](NVMLFanInfo info) {
		     return nvmlRead<uint, uint>(
			 info.dev,
			 [info](nvmlDevice_t dev, uint *value) {
	    ...
	    {[](nvmlDevice_t dev) {
			 ...
			 [](nvmlDevice_t dev, uint *value) {
				 return nvmlDeviceGetTemperatureThreshold(
				     dev, NVML_TEMPERATURE_THRESHOLD_SHUTDOWN, value);
			 },
			 std::nullopt);
	     },
		"°C", "Shutdown Temperature",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Shutdown Temperature"); }},
	    {[](nvmlDevice_t dev) {
		     return nvmlRead<uint, uint>(
			 dev,
			 [](nvmlDevice_t dev, uint *value) {
				 return nvmlDeviceGetTemperatureThreshold(
				     dev, NVML_TEMPERATURE_THRESHOLD_SLOWDOWN, value);
			 },
			 std::nullopt);
	     },
		"°C", "Slowdown Temperature",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Slowdown Temperature"); }}};
	...
	std::vector<UnspecializedReadable<nvmlDevice_t>> rawNVMLTempNodes = {
		...
		"°C", "Temperature",
		[](std::string uuid, nvmlDevice_t) { return md5(uuid + "Temperature"); }}};
	...
	std::vector<UnspecializedAssignable<NVCtrlFanInfo>> rawNVCTRLFanAssignables = {
	    {[=](NVCtrlFanInfo info) -> std::optional<AssignableInfo> {
		     NVCTRLAttributeValidValuesRec values;
		     if (XNVCTRLQueryValidTargetAttributeValues(m_dpy, NV_CTRL_TARGET_TYPE_GPU,
			     info.gpuIndex, info.fanIndex, NV_CTRL_GPU_COOLER_MANUAL_CONTROL,
			     &values) &&
			 (values.permissions & ATTRIBUTE_TYPE_WRITE)) {
		...
		[=](NVCtrlFanInfo info) -> std::optional<AssignmentArgument> {
			...
			auto valid = XNVCTRLQueryTargetAttribute(m_dpy, NV_CTRL_TARGET_TYPE_COOLER,
			    info.gpuIndex, info.fanIndex, NV_CTRL_THERMAL_COOLER_LEVEL, &value);
			if (valid)
				return value;
			return std::nullopt;
		},
	...
	// For both vectors: Find corresponding uuid from other vector, and append to combined
	// vector if uuid doesn't exist in it
	std::vector<NvidiaGPUDataOpt> optDataVec;

	for (auto nvctlGPU : nvctlGPUVec) {
		// Should never contain more than one despite the name
		auto sameGPUs = fp::keep_if(
		    [nvctlGPU](auto nvmlGPU) { return nvmlGPU.uuid == nvctlGPU.uuid; }, nvmlGPUVec);

		auto dev =
		    (!sameGPUs.empty()) ? std::optional(sameGPUs[0].identifier) : std::nullopt;
		optDataVec.push_back(
		    {.uuid = nvctlGPU.uuid, .index = nvctlGPU.identifier, .devHandle = dev});
	}

	for (auto nvmlGPU : nvmlGPUVec) {
		bool alreadyInVec = !fp::keep_if(
		    [nvmlGPU](auto nvOpt) { return nvmlGPU.uuid == nvOpt.uuid; }, optDataVec)
					 .empty();
		if (alreadyInVec)
			continue;

		auto sameGPUs =
		    fp::keep_if([nvmlGPU](auto nvctlGPU) { return nvmlGPU.uuid == nvctlGPU.uuid; },
			nvctlGPUVec);
		auto index =
		    (!sameGPUs.empty()) ? std::optional(sameGPUs[0].identifier) : std::nullopt;
		optDataVec.push_back(
		    {.uuid = nvmlGPU.uuid, .index = index, .devHandle = nvmlGPU.identifier});
	}
	...
	for (auto &nvOpt : optDataVec) {
		...
		match(nvOpt.devHandle)(
		    pattern(some(arg)) =
			[rawNVMLNodes, &gpuName](auto dev) {
				// Try to get GPU name from NVML
				char name[NVML_DEVICE_NAME_BUFFER_SIZE];
				if (nvmlDeviceGetName(dev, name, NVML_DEVICE_NAME_BUFFER_SIZE) ==
				    NVML_SUCCESS)
		...
		match(nvOpt.index)(
			...
			[&](auto index) {
				char *name = nullptr;
				if (XNVCTRLQueryStringAttribute(
					m_dpy, 0, index, NV_CTRL_STRING_PRODUCT_NAME, &name)) {
					gpuName = std::string(name);
					delete name;
				}
			},
	...
	auto gpuNodes = fp::transform(
	    [&](auto nvOpt) {
		    TreeNode<DeviceNode> gpuRoot =
			TreeNode(DeviceNode{.name = nvOpt.name, .hash = md5(nvOpt.uuid)});

		    // Has nodes from NVML and NVCtrl
		    auto utilRoot = TreeNode(DeviceNode{
			.name = "Utilizations", .hash = md5(nvOpt.uuid + "Utilizations")});

		    auto clockRoot =
			TreeNode{DeviceNode{.name = "Clocks", .hash = md5(nvOpt.uuid + "Clocks")}};

		    auto fanRoot =
			TreeNode(DeviceNode{.name = "Fans", .hash = md5(nvOpt.uuid + "Fans")});
		    ...
		    if_let(pattern(some(arg)) = nvOpt.devHandle) = [&](auto dev) {
			    ...
			    for (uint i = 0; i < nvmlFanCount(dev); i++) {
				    ...
				    for (auto &node :
					UnspecializedReadable<NVMLFanInfo>::toDeviceNodes(
				...
				.name = "Temperatures", .hash = md5(nvOpt.uuid + "Temperatures")});
			    auto tempNodes = UnspecializedReadable<nvmlDevice_t>::toDeviceNodes(
				rawNVMLTempNodes, dev, nvOpt.uuid);

			    auto tempInfoNodes =
				UnspecializedReadable<nvmlDevice_t>::staticToDeviceNodes(
		    ...
		    if_let(pattern(some(arg)) = nvOpt.index) = [&](auto index) {
			    for (auto &node : UnspecializedReadable<uint>::toDeviceNodes(
				     rawNVCTRLNodes, index, nvOpt.uuid))
				    gpuRoot.appendChild(node);
			    ...
			    for (int i = 0; i < nvctrlFanCount(index); i++) {
				    ...
				    for (auto &node :
					UnspecializedAssignable<NVCtrlFanInfo>::toDeviceNodes(
		    ...
		    if (maxFanCount > 1) {
			    for (uint i = 0; i < maxFanCount; i++) {
				    auto subFanRoot =
					TreeNode<DeviceNode>(DeviceNode{.name = std::to_string(i),
	...
	for (auto &gpuNode : gpuNodes)
		rootNode.appendChild(gpuNode);
